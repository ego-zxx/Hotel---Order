...
const ADMIN_PASSWORD = 'password123'; // Standard professional credentials
...
export function authenticateAdmin(user: string, pass: string): string | null {
  if (user === ADMIN_USERNAME && pass === ADMIN_PASSWORD) {
    ...
    const payload = btoa(JSON.stringify({ sub: ADMIN_USERNAME, role: 'admin', exp: Date.now() + 24 * 3600 * 1000 }));
...
export function verifyAdminToken(req: NextRequest): boolean {
  ...
  try {
    ...
    if (signature === MOCK_JWT_SECRET && payload.sub === ADMIN_USERNAME && payload.exp > Date.now()) {
      return true;
    }
  } catch (e) {
    return false;
