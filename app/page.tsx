...
export default function GuestOrderingApp() {
  ...
  const [screen, setScreen] = useState<
    "welcome" | "menu" | "cart" | "checkout" | "tracking"
  >(() => {
    if (typeof window !== "undefined") {
    ...
    const pollOrder = async () => {
      try {
        const res = await fetch(
          `/api/orders/${encodeURIComponent(activeOrderId)}`,
        );
        if (res.ok) {
  ...
  const handleSignOut = () => {
    if (
      window.confirm(
        "Do you want to clear your room session? Your cart will be cleared.",
      )
    ) {
    ...
    <div className="min-h-screen flex flex-col font-sans transition-colors duration-300 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 shadow-xs">
        <div className="max-w-4xl mx-auto px-4 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-sans text-base tracking-tight font-bold text-slate-900 dark:text-white leading-none">
                Menu
              </h1>
              <span className="text-[10px] tracking-wider text-slate-400 font-medium uppercase mt-0.5 block">
              ...
              >
                <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl p-8 md:p-10 shadow-sm border border-slate-200 dark:border-slate-800 text-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm mb-8 leading-relaxed">
                    Exquisite culinary creations, direct from our kitchen to
                    your door. Welcome to L&apos;Atelier.
                  </p>
                  ...
                  <form onSubmit={handleRoomSubmit} className="space-y-4">
                    ...
                    >
                      ...
                      <ChevronRight
                        size={16}
                        className="translate-y-[0.5px] group-hover:translate-x-1 transition-transform"
                      />
                    </button>
                  </form>
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-center justify-between pb-2">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    <span className="font-semibold text-indigo-600/90 dark:text-indigo-400/90">
                      ● Active
                    </span>
                ...
                {filteredMenuItems.length === 0 ? (
                  <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs">
                    ...
                    <h3 className="mt-4 font-sans text-lg font-bold text-slate-800 dark:text-slate-200">
                      No items found
                    </h3>
                    <p className="text-slate-400 text-xs mt-1">
                      Please try clearing your filters or searches.
                    </p>
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="relative p-2 bg-slate-800 dark:bg-slate-900 rounded-xl">
                          <ShoppingBag
                            size={18}
                            className="text-indigo-450 dark:text-indigo-400"
                          />
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-xs text-indigo-455 dark:text-indigo-300 font-medium">
                            {cartTotals.totalItems} selection
            ...
            {screen === "cart" && (
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
                  ...
                  <h2 className="font-sans text-base font-bold text-slate-900 dark:text-white">
                    Your Basket
                  </h2>
                </div>
                ...
                {cart.length === 0 ? (
                  <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4 shadow-xs">
                    ...
                    <h3 className="font-sans text-lg font-bold text-slate-800 dark:text-slate-200">
                      Your basket is empty
                    </h3>
                    <p className="text-xs text-slate-400">
                      Discover premium selections on our main dining menu.
                    </p>
                    <button
                      onClick={() => setScreen("menu")}
                          ...
                          <div className="flex-1 flex flex-col justify-between">
                            <div>
                              <div className="flex justify-between gap-2">
                                <h4 className="font-sans text-sm font-bold text-slate-900 dark:text-slate-100">
                                  {item.name}
                                </h4>
                            ...
                            <div className="flex items-center justify-between pt-1">
                              ...
                              <div className="flex items-center bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-0.5">
                                ...
                                <span className="px-2 text-xs text-slate-900 dark:text-slate-200 font-bold">
                                  {quantity}
                                </span>
                                <button
                                  onClick={() => updateQuantity(item.id, 1)}
              ...
              >
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-6 shadow-sm">
                  <div className="text-center border-b border-slate-200 dark:border-slate-800 pb-5">
                    <h2 className="font-sans text-xl font-bold text-slate-900 dark:text-white mb-1">
                      Confirm Order Delivery
                    </h2>
                    <p className="text-xs text-slate-400 font-medium">
                      Please review your culinary request before scheduling.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 text-xs">
                    ...
                    <div className="flex justify-between p-3.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl">
                      <div>
                        ...
                        <p className="text-sm font-sans font-bold text-slate-800 dark:text-white">
                    ...
                    <div className="space-y-2">
                      <p className="font-bold uppercase tracking-wider text-[9px] text-indigo-650 dark:text-indigo-400">
                        Gourmet Selections
                      </p>
                      <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                        {cart.map(({ item, quantity }) => (
                          <div
                            key={item.id}
                            className="flex justify-between items-center text-slate-600 dark:text-slate-350 font-medium"
                          >
                            <div className="flex items-center gap-2">
                              <span className="font-sans text-xs text-slate-500 bg-slate-100 dark:bg-slate-850 border dark:border-slate-800 rounded-md px-1.5 py-0.5">
                                {quantity}x
                              </span>
                              <span className="font-semibold text-slate-805 dark:text-slate-100">
                                {item.name}
                              </span>
                            </div>
              ...
              >
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-6 shadow-sm relative overflow-hidden">
                  ...
                  <div className="text-center border-b border-slate-200 dark:border-slate-800 pb-5">
                    ...
                    <p className="text-xs text-slate-400 font-medium">
                      Order is registered. Tracking details are synchronized
                      below.
                    </p>
                  </div>
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3 rounded-xl text-center">
                          ...
                          <p className="text-sm font-sans font-bold text-slate-800 dark:text-white">
                            Suite {trackingOrder.roomNumber}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 border border-slate-200/60 dark:border-slate-850 p-5 rounded-2xl bg-white dark:bg-slate-900 w-full animate-fadeIn">
                        <p className="font-bold uppercase tracking-wider text-[9px] text-indigo-600 dark:text-indigo-400 text-center pb-2 border-b border-slate-100 dark:border-slate-800/60">
                          Progress Timeline
                        </p>
                          ...
                          {[
                            {
                              ...
                              desc: "Our chef is reviewing your request.",
                            },
                            {
                              name: "Accepted",
                              label: "Order Accepted",
                              desc: "Acknowledged and approved for fulfillment.",
                            },
                            {
                              name: "Preparing",
                              label: "In the Kitchen",
                              desc: "Ingredients are being expertly crafted.",
                            },
                            {
                              name: "Out For Delivery",
                              label: "Transit Status",
                              desc: "Your warm tray is heading to your floor.",
                            },
                            {
                              name: "Delivered",
                              label: "Delivered",
                              desc: "Bon appétit! Enjoy your custom room service.",
                            },
                          ].map((step, idx, arr) => {
                            ...
                            return (
                              <div key={idx} className="relative">
                                {/* Dot Indicator */}
                                <div
                                  className={`absolute -left-[45px] top-0 w-4 h-4 rounded-full border-2 transition-all flex items-center justify-center ${
                                    isCancelled
                                ...
                                <div
                                  className={`${isCurrent ? "opacity-100 scale-[1.01]" : "opacity-60"} transition-all`}
                                >
      ...
      <footer className="bg-slate-950 text-slate-400 border-t border-slate-900 py-8 text-center text-xs mt-12 shrink-0">
        <div className="max-w-4xl mx-auto px-4 space-y-2">
          <p className="font-sans font-bold tracking-tight text-white text-sm">
            L&apos;Atelier Gourmand
          </p>
          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest leading-none">
            In-suite gourmet dining experiences • 24/7 dedicated service
          </p>
          <p className="text-[9px] text-slate-500 mt-2">
            &copy; {new Date().getFullYear()} Serene Boutique Hotel All culinary
            rights reserved.
          </p>
        </div>
      </footer>
