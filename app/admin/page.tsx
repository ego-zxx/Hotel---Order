...
export default function AdminDashboardApp() {
  ...
  // Active Tab View
  const [selectedTab, setSelectedTab] = useState<'dashboard' | 'orders' | 'menu' | 'categories' | 'analytics'>('dashboard');
  ...
  const [categoryFormType, setCategoryFormType] = useState<'create' | 'edit'>('create');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setAuthError(data.error || 'Invalid credentials mapping. Please try admin/password123.');
      }
    } catch (err) {
  ...
  // Clear Session Data
  const handleLogout = () => {
    if (window.confirm('Are you sure you want to sign out of the administrator command deck?')) {
      localStorage.removeItem('admin_auth_token');
      setToken(null);
      setOrders([]);
      setCategories([]);
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isEdit) {
          setProducts(prev => prev.map(p => p.id === selectedProductId ? savedItem : p));
          triggerSuccessAlert(`MenuItem &quot;${savedItem.name}&quot; modified`);
        } else {
          setProducts(prev => [...prev, savedItem]);
          triggerSuccessAlert(`Product &quot;${savedItem.name}&quot; created successfully`);
        }
  ...
  const handleDeleteProduct = async (prodId: string) => {
    if (!token || !window.confirm('Delete this food item permanently from the database?')) return;
    setActionLoading(true);
    try {
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isEdit) {
          setCategories(prev => prev.map(c => c.id === selectedCategoryId ? saved : c));
  ...
  const handleDeleteCategory = async (catId: string) => {
    if (!token || !window.confirm('Removing this category will not delete its associated items but they may require category re-assignment. Proceed?')) return;
    setActionLoading(true);
    try {
  ...
  const analyticsData = useMemo(() => {
    ...
    orders.forEach(o => {
      ...
      o.items.forEach(itm => {
        ...
        const correspondingProduct = products.find(p => p.id === itm.id || p.name === itm.name);
        const cat = correspondingProduct ? correspondingProduct.category : 'Breakfast';
        categoryCounts[cat] = (categoryCounts[cat] || 0) + itm.quantity;
        categoryRev[cat] = (categoryRev[cat] || 0) + (itm.price * itm.quantity);
      });
    });

    const categorySummary = Object.entries(categoryCounts).map(([name, count]) => ({
      name,
      count,
    ...
    const weekSalesArray = Object.entries(dailySalesMap).map(([day, val]) => ({
      day,
      revenue: val || (day === 'Sat' ? 240 : day === 'Sun' ? 310 : 150) // seed gorgeous mock defaults if table is empty
    }));
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="font-sans text-xl font-bold tracking-tight text-slate-105">L&apos;Atelier Backoffice</h1>
            <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-1 font-semibold">Management Command Deck</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-xs font-semibold">
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-slate-400 mb-2 font-bold">backoffice login</label>
              <input
                type="text"
                value={usernameInput}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-slate-400 mb-2 font-bold">administrative password</label>
              <input
                type="password"
                value={passwordInput}
          ...
          <div className="mt-8 pt-6 border-t border-slate-800 text-center">
            <Link href="/" className="text-slate-500 hover:text-slate-350 transition-colors text-xs font-bold uppercase tracking-wider">
      ...
      <aside className="w-full md:w-64 bg-slate-900 border-r border-slate-800 shrink-0 flex flex-col justify-between">
        <div className="p-6">
          <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-800/80">
            ...
            <div>
              <h1 className="font-sans text-sm tracking-tight font-bold text-white">Backoffice Desk</h1>
              <span className="text-[9px] tracking-widest text-indigo-400 font-bold uppercase">Command Desk Lounge</span>
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {selectedTab === 'dashboard' && (
            ...
            >
              {/* Header Title block */}
              <div>
                <h2 className="font-sans text-2xl font-bold tracking-tight text-white">Backoffice Overview</h2>
                <p className="text-xs text-slate-400 font-medium">Realtime room dining operations, catering updates and metrics.</p>
              </div>

              {/* Statistic Cards Row */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { title: 'Total Orders Today', value: stats.total, icon: ListOrdered, color: 'text-indigo-400 bg-indigo-500/10' },
                  { title: 'Revenue Today', value: `$${stats.revenue.toFixed(2)}`, icon: DollarSign, color: 'text-emerald-450 bg-emerald-500/10' },
                  { title: 'Pending Orders', value: stats.pending, icon: Clock, color: 'text-amber-400 bg-amber-400/10' },
                  { title: 'Delivered Orders', value: stats.delivered, icon: CheckCircle, color: 'text-sky-400 bg-sky-400/10' }
                ].map((stat, idx) => {
                  const Icon = stat.icon;
                  return (
                    <div key={idx} className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex items-center justify-between shadow-xs">
                      <div className="space-y-1.5">
                        <p className="text-slate-400 text-[10px] uppercase font-bold tracking-widest leading-none">{stat.title}</p>
                        <p className="font-sans text-2xl font-bold text-white tracking-tight">{stat.value}</p>
                      </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xs">
                <div className="flex justify-between items-center pb-2.5 border-b border-slate-800">
                  <h3 className="font-sans text-sm font-bold text-white">Active Suite Orders</h3>
                  <button 
                      ...
                      <tbody className="divide-y divide-slate-800/40">
                        {orders.slice(0, 5).map(o => (
                          <tr key={o.id} className="hover:bg-slate-800/20 text-slate-300">
                            <td className="py-3 px-4 font-mono font-bold text-indigo-455">{o.id}</td>
                            <td className="py-3 px-4 font-sans font-bold text-slate-150">Suite {o.roomNumber}</td>
                            ...
                            <td className="py-3 px-4">
                              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                                ...
                                o.status === 'Accepted' ? 'bg-indigo-500/10 text-indigo-400' :
                                o.status === 'Preparing' ? 'bg-orange-500/10 text-orange-400' :
                                o.status === 'Out For Delivery' ? 'bg-cyan-500/10 text-cyan-400' :
                            ...
                            <td className="py-3 px-4 text-right font-sans font-bold text-slate-100">${o.total.toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {selectedTab === 'orders' && (
            ...
            >
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <h2 className="font-sans text-2xl font-bold tracking-tight text-white">Fulfillment Queue</h2>
                  <p className="text-xs text-slate-400 font-medium">Accept, monitor and dispatch suites food packages.</p>
                </div>
                <button
                  onClick={() => fetchAdminData(token)}
              ...
              {orders.length === 0 ? (
                <div className="text-center py-20 bg-slate-900 border border-slate-800 rounded-2xl shadow-xs">
                  ...
                  <h3 className="mt-4 font-sans text-sm font-bold text-slate-250">Fulfilled Queue Empty</h3>
                  <p className="text-xs text-slate-500 mt-1 font-medium">Ready for incoming room orders.</p>
                </div>
              ) : (
                  ...
                  {orders.map(o => (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-slate-800/80 pb-4">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs font-bold text-indigo-400 bg-slate-950 border border-slate-800 px-2.5 py-0.5 rounded-md">{o.id}</span>
                            <span className="font-sans text-base font-bold text-white">Suite Room {o.roomNumber}</span>
                          </div>
                          <p className="text-[10px] text-slate-500 font-medium">
                            Time: {new Date(o.createdAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                          </p>
                        </div>

                        <div className="flex items-center gap-3">
                          <span className="text-xs text-slate-400 font-bold uppercase tracking-widest text-[10px]">Status:</span>
                          <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border ${
                            o.status === 'Pending' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                            o.status === 'Accepted' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' :
                            o.status === 'Preparing' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' :
                            o.status === 'Out For Delivery' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' :
                            o.status === 'Delivered' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                            'bg-rose-500/10 text-rose-450 border-rose-500/20'
                          }`}>
                            {o.status}
                          </span>
                          <span className="text-lg font-sans font-bold text-white shrink-0">${o.total.toFixed(2)}</span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-medium">
                        <div className="space-y-1.5">
                          <p className="text-[9px] font-bold uppercase tracking-wider text-indigo-400">Gourmet Selections</p>
                          <div className="space-y-1 bg-slate-950/40 p-3 rounded-xl border border-slate-800">
                            {o.items.map((itm, idx) => (
                              <div key={idx} className="flex justify-between font-medium text-slate-300">
                                <span>{itm.quantity}x {itm.name}</span>
                                <span className="font-sans font-bold text-slate-200">${itm.price.toFixed(2)}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <p className="text-[9px] font-bold uppercase tracking-wider text-slate-500">Culinary Directives</p>
                          <div className="bg-slate-950/40 p-3 rounded-xl border border-slate-800 h-full min-h-[50px] font-medium text-slate-400 italic">
                            {o.notes ? `“${o.notes}”` : 'No custom food preparation instructions.'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-805/60 justify-end">
                        {o.status === 'Pending' && (
                          <button
                            onClick={() => updateOrderStatusAction(o.id, 'Accepted')}
                        ...
                        {o.status === 'Accepted' && (
                          <button
                            onClick={() => updateOrderStatusAction(o.id, 'Preparing')}
                        ...
                        {o.status === 'Preparing' && (
                          <button
                            onClick={() => updateOrderStatusAction(o.id, 'Out For Delivery')}
                        ...
                        {o.status === 'Out For Delivery' && (
                          <button
                            onClick={() => updateOrderStatusAction(o.id, 'Delivered')}
                        ...
                        {o.status !== 'Delivered' && o.status !== 'Cancelled' && (
                          <button
                            onClick={() => updateOrderStatusAction(o.id, 'Cancelled')}
          ...
          {selectedTab === 'menu' && (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="font-sans text-2xl font-bold tracking-tight text-white">Culinary Catalog</h2>
                  <p className="text-xs text-slate-400 font-medium">Add, edit and manage available suite recipes.</p>
                </div>
              ...
              {products.length === 0 ? (
                <div className="text-center py-20 bg-slate-900 border border-slate-800 rounded-2xl shadow-xs">
                  ...
                  <h3 className="mt-4 font-sans text-sm font-bold text-slate-250">Catalog Empty</h3>
                  <p className="text-xs text-indigo-400 mt-1 font-semibold">Add your first hotel dish selections today.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {products.map(item => (
                    ...
                    >
                      <div className="relative w-24 h-24 bg-slate-950 rounded-xl overflow-hidden shrink-0 border border-slate-800/80">
                        <Image 
                        ...
                        {!item.available && (
                          <div className="absolute inset-0 bg-slate-950/80 flex items-center justify-center">
                            <span className="text-[8px] bg-rose-950 font-bold border border-rose-900 text-rose-400 px-1.5 py-0.5 rounded uppercase">SOLDOUT</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 flex flex-col justify-between font-medium">
                        <div>
                          <div className="flex justify-between gap-1 items-start">
                            <h4 className="font-sans text-sm font-bold text-slate-100">{item.name}</h4>
                            <span className="text-xs font-bold text-indigo-400 font-sans shrink-0">${item.price.toFixed(2)}</span>
                          </div>
                          <span className="text-[9px] bg-slate-950 border border-slate-800 text-slate-400 px-2 py-0.5 rounded-full inline-block mt-1 font-bold uppercase">{item.category}</span>
                          <p className="text-[10px] text-slate-400 mt-2 leading-relaxed line-clamp-2 font-medium">{item.description}</p>
                        </div>
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                const res = await fetch(`/api/admin/products/${encodeURIComponent(item.id)}`, {
                                  method: 'PUT',
                                  headers: {
                                    'Content-Type': 'application/json',
                                    Authorization: `Bearer ${token}`
                                ...
                                if (res.ok) {
                                  setProducts(prev => prev.map(p => p.id === item.id ? { ...p, available: nextState } : p));
                                  triggerSuccessAlert(`&ldquo;${item.name}&rdquo; is now ${nextState ? 'Available' : 'Unavailable'}`);
                                }
          ...
          {selectedTab === 'categories' && (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="font-sans text-2xl font-bold tracking-tight text-white">Category Management</h2>
                  <p className="text-xs text-slate-400 font-medium">Classify your hotel culinary crafts logically.</p>
                </div>
              ...
              {categories.length === 0 ? (
                <div className="text-center py-20 bg-slate-900 border border-slate-800 rounded-2xl shadow-xs">
                  ...
                  <h3 className="mt-4 font-sans text-sm font-bold text-slate-250">No categories loaded</h3>
                  <p className="text-xs text-slate-500 mt-1 font-medium">Define categories for beautiful filtering.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {categories.map(cat => {
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-1">
                          <h4 className="font-sans text-base font-bold text-slate-100">{cat.name}</h4>
                          <p className="text-[10px] text-indigo-400 font-bold uppercase">{itemCount} Selections assigned</p>
                        </div>
          ...
          {selectedTab === 'analytics' && (
            ...
            >
              <div>
                <h2 className="font-sans text-2xl font-bold tracking-tight text-white">Visual Operations Analytics</h2>
                <p className="text-xs text-slate-400 font-medium">Detailed metric streams and statistical distribution charts.</p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 lg:col-span-2 space-y-4 shadow-xs">
                  <h3 className="font-sans text-sm font-bold text-white">Weekly Orders Revenue Trend</h3>
                  
                  {/* Custom Line Grid Chart in SVG */}
                  <div className="h-64 w-full relative pt-2">
                    <svg className="w-full h-full" viewBox="0 0 500 220">
                      {/* Grid Lines */}
                      <line x1="30" y1="30" x2="480" y2="30" stroke="#1e293b" strokeDasharray="3" />
                      <line x1="30" y1="80" x2="480" y2="80" stroke="#1e293b" strokeDasharray="3" />
                      <line x1="30" y1="130" x2="480" y2="130" stroke="#1e293b" strokeDasharray="3" />
                      <line x1="30" y1="180" x2="480" y2="180" stroke="#1e293b" />

                      {/* Line Paths & Markers */}
                      {(() => {
                        const maxVal = Math.max(...analyticsData.weekSales.map(d => d.revenue), 100);
                        const points = analyticsData.weekSales.map((dayData, idx) => {
                          const x = 30 + (idx * 75);
                          const y = 180 - ((dayData.revenue / maxVal) * 130);
                          return { x, y, label: dayData.day, val: dayData.revenue };
                        });
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xs">
                  <h3 className="font-sans text-sm font-bold text-white">Revenue by Category</h3>

                  <div className="space-y-4">
                    {analyticsData.categorySummary.map((cat, idx) => {
                      const maxRev = Math.max(...analyticsData.categorySummary.map(c => c.revenue), 1);
                      const barWidth = `${Math.max((cat.revenue / maxRev) * 100, 3)}%`;
                      return (
                        <div key={idx} className="space-y-1 text-xs font-semibold">
                          <div className="flex justify-between items-center text-slate-300">
                            <span>{cat.name}</span>
                            <span className="font-mono font-bold text-indigo-400">${cat.revenue.toFixed(0)}</span>
                          </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 lg:col-span-3 space-y-4 shadow-xs">
                  <h3 className="font-sans text-sm font-bold text-white">Signature Dish sales ranking</h3>
                      ...
                      <table className="w-full text-left text-xs font-semibold">
                        <thead>
                          <tr className="text-slate-500 border-b border-slate-800/80 pb-2 uppercase tracking-widest text-[9px]">
                            ...
                            <th className="py-2.5 px-2 text-center font-bold">Quantities Ordered</th>
                            <th className="py-2.5 px-2 text-right font-bold">Invoice Sum</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/40">
                          {analyticsData.productsOrderedSorted.map((item, idx) => (
                            <tr key={idx} className="hover:bg-slate-800/10 text-slate-300">
                              <td className="py-3 px-2 flex items-center gap-2">
                                ...
                                <span className="font-bold text-slate-150">{item.name}</span>
                              </td>
                              <td className="py-3 px-2 text-center text-slate-400 font-mono font-bold">{item.qty} portions</td>
                              <td className="py-3 px-2 text-right font-sans text-indigo-400 font-bold">${item.rev.toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <h3 className="font-sans text-lg font-bold text-white">
                  {productFormType === 'create' ? 'Add Exquisite Plate Selection' : 'Refine Recipe Selection'}
                </h3>
                <button 
              ...
              <form onSubmit={handleProductSubmit} className="space-y-4 text-xs font-semibold">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[9px] uppercase tracking-wider text-slate-400 mb-1.5 font-bold">dish title name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[9px] uppercase tracking-wider text-slate-400 mb-1.5 font-bold">Portion Rate Price ($)</label>
                    <input
                      type="number"
                      step="0.01"
                ...
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-slate-400 mb-1.5 font-bold">gourmet classification category</label>
                  <select
                ...
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-slate-400 mb-1.5 font-bold">Illustration Image URL</label>
                  <input
                    type="url"
                    ...
                    required
                  />
                  <p className="text-[9px] text-slate-500 mt-1 font-medium">Accepts any online image address protocol.</p>
                </div>

                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-slate-400 mb-1.5 font-bold">Flavor notes description</label>
                  <textarea
        ...
        {categoryModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
            ...
            >
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <h3 className="font-sans text-lg font-bold text-white">
                  {categoryFormType === 'create' ? 'Draft New Category' : 'Rename Category Classification'}
                </h3>
                <button 
              ...
              <form onSubmit={handleCategorySubmit} className="space-y-4 text-xs font-semibold">
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-slate-400 mb-1.5 font-bold">Classification label</label>
                  <input
                    type="text"
